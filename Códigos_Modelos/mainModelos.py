...
from tensorflow.keras.layers import Dense, Dropout, GRU, LSTM, Conv1D, Flatten  # Importa camadas para a rede neural. Dense cria uma camada totalmente conectada, Dropout ajuda a prevenir overfitting, e LSTM é uma camada recorrente utilizada para processamento de sequências (útil em séries temporais).
...
class Modelo:
    def Calculos (rede, dias, execucoes, epocas, lista_arquivos):
        tempo_inicial_total = time.time()

        # Diretório raiz do projeto
        raiz = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        caminho_pasta_dados = os.path.join(raiz, "Base_dados")
        caminho_pasta_rede = os.path.join(raiz, "Bases_Rede")

        # caminho_pasta = 'Base_dados'
        lista_arquivos = [arquivo for arquivo in os.listdir(caminho_pasta_dados) if os.path.isfile(os.path.join(caminho_pasta_dados, arquivo))]
        ...
        redes = ["GRU", "LSTM", "Conv1d"]

        if rede not in redes:
            raise ValueError(f"Rede Não Suportada: {rede}. Escolha entre {redes}.")

        for a in range(execucoes):
            tempo_inicial_execucoes = time.time()
            print(f"\nLoop {a+1}/{execucoes} (Rede {rede}, {dias} dias, {epocas} épocas)")
            for arq in lista_arquivos:
                tempo_inicial_loop = time.time()

                # Carregar dados
                df = pd.read_csv(f'{caminho_pasta_dados}/{arq}', parse_dates=['Date'])
                df = df.sort_values('Date')

                arq = arq[:-4]

                # Defina o caminho da nova pasta que você quer criar
                if rede == "LSTM" or rede == "GRU":
                    caminho = f'{caminho_pasta_rede}/{dias}_dias/Resultados_{rede}_Dropout_Dense/{arq}'
                elif rede == "Conv1d":
                    caminho = f'{caminho_pasta_rede}/{dias}_dias/Resultados_{rede}_Dropout_Flatten_Dense/{arq}'
                ...
                for i in range(dias, len(base_treinamento_normalizada)):
                ...
                if rede == "GRU":
                    # 1ª Camada de GRU
                    regressor.add(GRU(units=120, return_sequences=True, input_shape=(X.shape[1], 1)))
                    ...
                    raise ValueError("Arquitetura de Rede Desconhecida.")
                ...
                treinamento = regressor.fit(X, y, epochs=epocas, batch_size=32)
                ...
                plt.title(f"Previsões dos Preços das Ações {arq[5:]} com {rede}")
                plt.xlabel("Tempo")
                plt.ylabel("Valor")
                ...
                results = Resultados(mae, mse, r2, mape, acuracia, valor_mean_absolute_error, valor_loss, tempo_total_loop)
                if arq == "hist_BBAS3":
                    resultadosBBAS3.append(results)
                elif arq == "hist_EA":
                    resultadosEA.append(results)
                elif arq == "hist_NFLX":
                    resultadosNFLX.append(results)
                elif arq == "hist_NVDA":
                    resultadosNVDA.append(results)
                elif arq == "hist_PETR4":
                    resultadosPETR4.append(results)
                elif arq == "hist_TAEE11":
                    resultadosTAEE11.append(results)

                if rede == "LSTM" or rede == "GRU":
                    nome_arquivo_resultado = f'{caminho}/Resultados_Previsoes_{arq}_{epoca}_{rede}_Dropout_Dense.csv'
                    count_res = 1

                    while os.path.exists(nome_arquivo_resultado):
                        nome_arquivo_resultado = f'{caminho}/Resultados_Previsoes_{arq}_{epoca}_{rede}_Dropout_Dense_{count_res}.csv'
                        count_res += 1

                    nome_arquivo_foto = f'{caminho}/Previsao_{arq}_{epoca}_{rede}_Dropout_Dense.png'
                    count_fot = 1

                    while os.path.exists(nome_arquivo_foto):
                        nome_arquivo_foto = f'{caminho}/Previsao_{arq}_{epoca}_{rede}_Dropout_Dense.png_{count_fot}.png'
                        count_fot += 1

                else:
                    nome_arquivo_resultado = f'{caminho}/Resultados_Previsoes_{arq}_{epoca}_{rede}_Dropout_Flatten_Dense.csv'
                    count_res = 1

                    while os.path.exists(nome_arquivo_resultado):
                        nome_arquivo_resultado = f'{caminho}/Resultados_Previsoes_{arq}_{epoca}_{rede}_Dropout_Flatten_Dense_{count_res}.csv'
                        count_res += 1
                    
                    nome_arquivo_foto = f'{caminho}/Previsao_{arq}_{epoca}_{rede}_Dropout_Flatten_Dense.png'
                    count_fot = 1

                    while os.path.exists(nome_arquivo_foto):
                        nome_arquivo_foto = f'{caminho}/Previsao_{arq}_{epoca}_{rede}_Dropout_Flatten_Dense_{count_fot}.png'
                        count_fot += 1
