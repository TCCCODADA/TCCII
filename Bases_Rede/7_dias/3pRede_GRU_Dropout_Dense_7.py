...
for arq in lista_arquivos:
    ...
    caminho = f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}'
    ...
    if os.path.exists(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Training.csv'):
        os.remove(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Training.csv')
    base_70.to_csv(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Training.csv', index=False)

    # Salvar os últimos 30% em um novo arquivo CSV
    if os.path.exists(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Test.csv'):
        os.remove(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Test.csv')
    base_30.to_csv(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Test.csv', index=False)

    base = pd.read_csv(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Training.csv')
    ...
    base_teste = pd.read_csv(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/{arq}_Test.csv')
    ...
    nome_arquivo_resultado = f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/Resultados_Previsoes_{arq}_{epoca}_{camadas[0]}_{camadas[1]}_{camadas[2]}.csv'
    count_res = 1

    while os.path.exists(nome_arquivo_resultado):
        nome_arquivo_resultado = f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/Resultados_Previsoes_{arq}_{epoca}_{camadas[0]}_{camadas[1]}_{camadas[2]}_{count_res}.csv'
        count_res += 1

    resultados.to_csv(nome_arquivo_resultado, index=False)

    nome_arquivo_foto = f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/Previsao_{arq}_{epoca}_{camadas[0]}_{camadas[1]}_{camadas[2]}.png'
    count_fot = 1

    while os.path.exists(nome_arquivo_foto):
        nome_arquivo_foto = f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/Previsao_{arq}_{epoca}_{camadas[0]}_{camadas[1]}_{camadas[2]}_{count_fot}.png'
        count_fot += 1
        
    plt.savefig(nome_arquivo_foto, dpi=300, bbox_inches='tight')
    # plt.savefig(f'Bases_Rede/7_dias/Resultados_GRU_Dropout_Dense/{arq}/Previsao_{arq}.png', dpi=300, bbox_inches='tight')  # Salva a imagem com alta resolução   
