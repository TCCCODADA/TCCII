...
from Códigos.Resultados import Resultados
...
class Gru:
    def GruCalcs (dias, execucoes, epocas, lista_arquivos):
        ...
        for a in range(execucoes):
            ...
            for arq in lista_arquivos:
                ...
                caminho = f'Bases_Rede/{dias}_dias/Resultados_GRU_Dropout_Dense/{arq}'
                ...
                for i in range(dias, len(base_treinamento_normalizada)):  # 30 Preços Anteriores para prever o preço atual
                ...
                nome_arquivo_resultado = f'{caminho}/Resultados_Previsoes_{arq}_{epoca}_GRU_Dropout_Dense.csv'
                count_res = 1

                while os.path.exists(nome_arquivo_resultado):
                    nome_arquivo_resultado = f'{caminho}/Resultados_Previsoes_{arq}_{epoca}_GRU_Dropout_Dense_{count_res}.csv'
                    count_res += 1

                resultados.to_csv(nome_arquivo_resultado, index=False)

                nome_arquivo_foto = f'{caminho}/Previsao_{arq}_{epoca}_GRU_Dropout_Dense.png'
                count_fot = 1

                while os.path.exists(nome_arquivo_foto):
                    nome_arquivo_foto = f'{caminho}/Previsao_{arq}_{epoca}_GRU_Dropout_Dense.png_{count_fot}.png'
                    count_fot += 1
