...
# Caminho absoluto para a pasta Base_dados
caminho_pasta_bases = os.path.abspath(os.path.join("..", "Base_dados"))

# DADOS HISTORICOS
def hist_data(data_inicio, data_fim, lista_acoes): # FUNCAO PARA OBTERMOS OS DADOS HISTORICOS DAS ACOES INDICADAS NA LISTA

    for acao in lista_acoes:    
        data = yf.download(acao, start=data_inicio, end=data_fim, group_by="column", auto_adjust=False) # baixa os dados historicos das acoes colocadas na lista "lista_acoes"
        data.columns = [col.split(" ")[-1] for col in data.columns]
        # Verifica se os dados foram baixados corretamente
        if data.empty:
            print(f"Nenhum dado encontrado para a ação: {acao}")
            continue
        ...
        data_df_pandas.to_csv(f"{caminho_pasta_bases}/hist_{acao}.csv") # transformando o df em um csv e salvando na pasta "Dados historicos"

# SMA
def MM_sma(lista_acoes):

    for acao in lista_acoes:
        df_sma = pd.read_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")
        ...
        df_sma.to_csv(f"{caminho_pasta_bases}/hist_{acao}.csv") # transformando o df em um csv e salvando na pasta "Dados historicos"

# EMA
def MM_ema(lista_acoes): # EMA tambem conhecida como Media Movel Exponencial

    for acao in lista_acoes:
        df_ema = pd.read_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")
        df_ema.ta.ema(lenght = 10,
                      append = True)
        
        df_ema.to_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")
...
def MM_rsi(lista_acoes): # AQUI OBTEMOS O INDICADOR RSI (RELATIVE STRENGTH INDEX ou INDICE DE FORCA RELATIVA)
    ...
    for acao in lista_acoes:
        df_rsi = pd.read_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")
        df_rsi["RSI_10"] = ta.rsi(close = df_rsi["Close"], # UTILIZA A COLUNA DE FECHAMENTO
                                length = 10,
                                append = True)
    
        df_rsi.to_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")
...
def MM_macd(lista_acoes):
    ...
    for acao in lista_acoes:
        df = pd.read_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")     
        df_MM_MACD = pd.read_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")  
        ...
        df.to_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")      

# Bollinger Bands
def MM_bb(lista_acoes): # BOLINGER BANDS nos ajuda a medir a volatilidade do ativo

    for acao in lista_acoes:
        df_bb = pd.read_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")   
        ...
        df_bb.to_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")
...
def MM_estc(lista_acoes): # AQUI OBTEMOS O INDICADOR ESTOCASTICO, GERANDO DUAS COLUNAS
    ...
    for acao in lista_acoes:
        df_estc = pd.read_csv(f"{caminho_pasta_bases}/hist_{acao}.csv")
